...
/**
 * A Promise/A compatible implmentation of promise.
...
Promise.prototype = {
   ...
   * ```
   *
   * If callback returns an intances of error the returned promise if rejected
   * with the given. In opposite if the errback does'nt return an error, the
   * callback is called.
   *
   * ```js
   * new Promise()
  ...
  /**
   * Add callback called on resolve state or if already resolved.
   *
   * @param {Function} callback
   * @param {Object}   context
   */
  ...
  /**
   * Add errback called on reject state or if already rejected.
   *
  ...
  /**
   * Add a progress callabck called when `progress` is emited.
  ...
  /**
   * Cancel the promise :cannot be rejected of resolved.
   */
  cancel: function() {
    this._events.rejected.disable();
    this._events.resolved.disable();
...
/**
 * Test if an object is a promise = test if it has #then method.
 * @static
 *
 ...
 * @static
 *
 * If the given object is a promise, it simply returns the same object, if
 * it's a value it returns a new resolved promise object.
 *
 *
...
/**
 * Returns a promise resolved when all given promises are completed and if at
 * least a given number of them are have been resolved.
 *
 * If too many promises are rejected to fulfill the "atLeast requirement", the
 * returned promise is immediately rejected.
 *
 * The resolved and rejected arguments are an array of the resolved promises and
 * an array of the rejected promises.
 *
 * @param  {Array}  promises  - array of promises to test
 * @param  {Number} toResolve - number of promises that need to be resolved to
 *                              to resolve the returned promise
 *
 * @return {Promise} promises that will be resolved
 */
...
/**
 * Returns a promise resolved as soon as a given number of the given promises
 * are resolved.
 *
 * If too many promises are rejected to fulfill the "atLeast requirement", the
 * returned promise is immediately rejected.
...
Promise.whenSome = function(promises, toResolve) {
   ...
   *
   * If one of the promises is rejected the returned promise is rejected.
   *
