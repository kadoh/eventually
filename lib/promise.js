...
Promise.whenSome = function(promises, toResolve) {
  ...
  /**
   * Returns a promise that resolves the result of the map operation on the
   * results of the given resolved promises.
   *
  ...
  Promise.whenMap = function(promises, map) {
    ...
    // construct promises that resolves the maped result
    var maped_promises = promises.map(function(promise) {
      //return a new promise
      return promise.pipe(function() {
        return map.apply(promise, arguments);
      });
    });

    // transform whenAll generated promise into a promise that
    // resolves the result array
    return Promise.whenAll(maped_promises)
                  .pipe(function(resolved) {
                    return resolved.map(function(promise) {
                      return promise.resolvedArguments()[0];
                    });
                  });
  };