 ...
 *
 * @event state [args]  - emitted when state changes
 *
...
StateEventEmitter.prototype = {
  ...
  /**
   * Call a function when the state changes. The handler function will be called
   * with the state as parameter.
   * @param {Function} handler   - the function to be called when state changes
   * @param {Object}   [context] - context in which the callback function will
   *                              be called
   * @param {Object}   [options] - EventEmmitter#on options
   * @return {self} this
   */
  onStateChange: function(handler, context, options) {
    seeLazyInit(this);
    this.on('state_change', handler, context, options);
    return this;
  }
};

// event emitter inheritence
